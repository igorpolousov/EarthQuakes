...
## Section 3: Add the map view
    ...
    4. Нажмите и удерживайте клавишу Option, перемещая указатель мыши по симулятору Xcode, чтобы имитировать жесты pinch to zoom
    5. Сделаем настройки map view так, чтобы они показывали нужный нам регион
    6. Добавляем location и tint color и удалим preview в данном файле потому что будем пользоваться preview в файле QuakeDetail.swift
    7. В фале QuakeDetail.swift добавим QuakeDetailMap view, который будет отображаться если доступные данные с координатами и поместим его в самый верх Vstack
    8. В файле QaukeDetailMap.swift добавим структуру, чтобы переводить и хранить данные местоположения как CLLocationCoordinate2D.
    9. Добавим свойство place и инициализатор чтобы инициализировать place с location
    10. Изменим Map view чтобы можно было добавить аннотации 
